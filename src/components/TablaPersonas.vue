    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="persona in personas" :key="persona.id">
          ...
          <td v-else>
            <button class="btn btn-info ml-2 mx-2" @click="editarPersona(persona)">✏️ Editar</button>
            <button class="btn btn-danger mx-2" v-on:click="$emit('delete-persona', persona.id)">🗑️ Eliminar</button>
          </td>
        </tr>
      </tbody>
    </table>
